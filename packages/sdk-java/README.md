...
The RDMind Java SDK is a minimum experimental SDK for programmatic access to RDMind functionality. It provides a Java interface to interact with the RDMind CLI, allowing developers to integrate RDMind capabilities into their Java applications.
...
```gradle
implementation 'com.xiaohongshu:rdmind-sdk:{$version}'
```
...
mvn exec:java -Dexec.mainClass="com.xiaohongshu.rdmind.cli.example.SimpleTest"

# Run all examples (recommended)
mvn exec:java -Dexec.mainClass="com.xiaohongshu.rdmind.cli.example.QuickStartExample"
...
The simplest way to use the SDK is through the `RDMindCli.simpleQuery()` method:
...
public static void runTransportOptionsExample() {
    ...
    List<String> result = RDMindCli.simpleQuery("who are you, what are your capabilities?", options);
    result.forEach(logger::info);
}
```
...
public static void runStreamingExample() {
    RDMindCli.simpleQuery("who are you, what are your capabilities?",
...
other examples see src/test/java/com/xiaohongshu/rdmind/cli/example

## Architecture

The SDK follows a layered architecture:

- **API Layer**: Provides the main entry points through `RDMindCli` class with simple static methods for basic usage
- **Session Layer**: Manages communication sessions with the RDMind CLI through the `Session` class
...
The `TransportOptions` class allows configuration of how the SDK communicates with the RDMind CLI:

- `pathToRDMindExecutable`: Path to the RDMind CLI executable
...
- **Session creation**: Use `RDMindCli.newSession()` to create a new session with custom options
...
- **Thread Naming**: "rdmind_cli-pool-{number}"
...
### Q: Do I need to install the RDMind CLI separately?
