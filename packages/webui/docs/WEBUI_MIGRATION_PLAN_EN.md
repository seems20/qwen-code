...
1. Extract components from `vscode-ide-companion/src/webview/` into an independent `@rdmind/webui` package
...
```typescript
// @rdmind/webui/src/context/PlatformContext.ts
...
```javascript
// @rdmind/webui/tailwind.preset.js
module.exports = {
  theme: {
    extend: {
      colors: {
...
```
Developer ──► @rdmind/webui ──► vscode-ide-companion
  │              │                      │
...
// After: Update import
import { FileIcon } from '@rdmind/webui';
// or import { FileIcon } from '@rdmind/webui/icons';
```
...
1. **Tailwind Class Name Tree Shaking**
   ...
   - Solution: Consumer's `content` config needs to include `node_modules/@rdmind/webui`
...
│     └── Edit files in @rdmind/webui/src/                     │
│                                                                  │
...
│     └── import { Component } from '@rdmind/webui'            │
...
1. **Single Source of Truth**: All UI components live in `@rdmind/webui`
