...
const MOCK_DATA: InsightData = {
  ...
  qualitative: {
    impressiveWorkflows: {
      intro:
        "You're a highly active RDMind user with 314 sessions over 67 days, achieving strong results in test creation, PR workflows, and debugging with a 68% success rate on clear tasks.",
      impressive_workflows: [
        ...
        },
        {
          ...
          description:
            "You've mastered the PR creation workflow, successfully creating pull requests using existing templates multiple times. You know how to leverage RDMind to format PRs properly in English and complete this task efficiently, often in single responses with multiple coordinated tool calls.",
        },
        {
          title: 'Systematic Debugging and Refactoring',
          description:
            "You effectively use RDMind for debugging failing tests, successfully identifying issues like missing KeypressProvider context and ink mock setup problems. You also completed clean refactoring tasks like migrating openFileInBrowser to use the 'open' package, with all checks passing on the first attempt.",
        },
      ],
    },
    projectAreas: {
    ...
    frictionPoints: {
      ...
      categories: [
        ...
        },
        {
          ...
          description:
            "You encountered buggy code generation in 5 instances where RDMind's initial implementations were incorrect, forcing you to spend additional time iterating on fixes before achieving working solutions.",
          examples: [
        ...
        },
        {
          ...
          description:
            "You experienced situations where RDMind's tools or skills failed to execute properly, forcing manual workarounds and excessive exploration that extended task completion time.",
          examples: [
    ...
    memorableMoment: {
      headline:
        "User paused mid-work to verify RDMind's corporate identity—\"Are you Alibaba's model or Zhipu's GLM?\"",
      detail:
        "During what appears to be routine development work, a user stopped everything to ask RDMind to confirm its corporate origins. RDMind correctly identified itself as Alibaba's model, not Zhipu's GLM. A rare moment of AI identity verification in the wild.",
    },
    improvements: {
      RDMind_md_additions: [
        {
          ...
          prompt_scaffold:
            'Create new ## Code Review Standards section in RDMind.md',
        },
        {
          addition:
          ...
          prompt_scaffold:
            'Create new ## Testing Conventions section in RDMind.md',
        },
        {
          addition:
          ...
          prompt_scaffold: 'Create new ## PR Workflow section in RDMind.md',
        },
      ],
      features_to_try: [
        {
        ...
        },
        {
          ...
          example_code:
            'Create .rdmind/skills/review/SKILL.md:\n```\nReview code for:\n1. Test coverage for new features\n2. Error handling in async functions\n3. Security implications\n4. Breaking changes\n```',
        },
        {
          feature: 'Headless Mode',
          one_liner: 'Run RDMind non-interactively for batch operations',
          why_for_you:
            'With 166 run_shell_command calls and repetitive tasks like PR creation, you could automate routine workflows',
          example_code:
            '# Create PR from latest commits\nrdmind -p "Create a PR for recent commits using .github/PULL_REQUEST_TEMPLATE.md"\n\n# Batch test generation\nrdmind -p "Generate tests for src/DataProcessor.ts"',
        },
        {
          feature: 'Task Agents',
          ...
          example_code:
            'Ask RDMind: "Use an agent to explore the test infrastructure and identify all test utilities available"',
        },
      ],
      usage_patterns: [
        {
        ...
        },
        {
          ...
          copyable_prompt:
            'Create .rdmind/skills/pr/SKILL.md with your PR template requirements, then just type /pr',
        },
        {
        ...
        },
        {
          ...
          suggestion:
            'Add testing conventions to RDMind.md to avoid iterative mock fixes',
          detail:
            '5 buggy_code friction points mostly came from test mock implementation issues. Documenting your testing patterns in RDMind.md would help RDMind generate correct tests on the first attempt.',
          copyable_prompt:
            "Add to RDMind.md: 'When generating tests, use vi.mock() before vi.mocked() and always include required providers'",
        },
      ],
    },
    interactionStyle: {
      narrative:
        'Your interaction pattern shows **high-frequency, low-intensity engagement** with RDMind. With 314 sessions over just 68 days (averaging 4-5 sessions daily) and 58% of analyzed sessions being "warmup_minimal," you treat RDMind as a readily-available assistant you check in with frequently but often don\'t have specific tasks ready. You say "hi" or "nihao" and wait to see what happens, suggesting you keep RDMind accessible as a background tool rather than planning extensive work sessions.\n\nWhen you do have actual work, you\'re **iterative and feedback-driven rather than specification-heavy**. Examples: when creating PRs, you accepted an initial implementation that created empty content, then reported the issue for RDMind to fix; when implementing ESC cancellation support, you tested the code, discovered the signal wasn\'t being passed through, and reported the specific bug for correction; when requesting tests for DataProcessor, you worked through multiple rounds of fixing mock implementation issues. You don\'t provide exhaustive upfront requirements—you point RDMind at a problem, see what it produces, and course-correct.\n\n**You trust RDMind to explore autonomously but intervene when things go wrong.** Your tool usage (209 read_file calls, 166 shell commands, 98 edits) shows you let RDMind investigate and modify freely. The friction data reveals low rejection rates (0 user_rejected_action, 0 excessive_changes) and zero misunderstood requests, indicating you give RDMind space to work. However, 5 instances of buggy code required your feedback to fix, and you clearly communicate specific issues when they arise ("abort() was called but execution didn\'t stop"). You\'re a collaborative debugger who provides precise problem descriptions rather than vague complaints.',
      key_pattern:
        'You use RDMind as a high-frequency, low-commitment tool with iterative feedback loops—many brief check-ins with "hi" or minimal interaction, and when real work happens, you prefer to test implementations and report specific issues for correction rather than providing comprehensive upfront specifications.',
    },
    atAGlance: {
      whats_working:
