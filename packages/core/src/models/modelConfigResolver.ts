...
import { decryptApiKey } from '../utils/apiKeyEncryption.js';
import {
  resolveField,
...
export function resolveModelConfig(
  ...
  // Special handling for XHS SSO - credentials are stored encrypted in settings
  if (authType === AuthType.XHS_SSO) {
    return resolveXhsSsoConfig(input, warnings);
  }
...
/**
 * Special resolver for XHS SSO authentication.
 * XHS SSO stores encrypted credentials in settings file.
 * The apiKey, baseUrl, and model are all read from settings and the apiKey is decrypted.
 */
function resolveXhsSsoConfig(
  input: ModelConfigSourcesInput,
  warnings: string[],
): ModelConfigResolutionResult {
  ...
  if (model) {
    sources['model'] = settingsSource('model.name');
  } else {
    warnings.push('XHS SSO: No model configured in settings.');
  }

  // Read baseUrl from settings
  const baseUrl = settings?.baseUrl || '';
  if (baseUrl) {
    sources['baseUrl'] = settingsSource('security.auth.baseUrl');
  } else {
    warnings.push('XHS SSO: No baseUrl configured in settings.');
  }

  // Read and decrypt apiKey from settings
  let apiKey: string | undefined;
  if (settings?.apiKey) {
    // Decrypt the API key (stored encrypted in settings.json)
    apiKey = decryptApiKey(settings.apiKey);
    sources['apiKey'] = settingsSource('security.auth.apiKey (decrypted)');
  } else {
    warnings.push('XHS SSO: No apiKey configured in settings.');
  }
