...
export class SkillManager {
  ...
  /**
   * Gets the base directories for skills at a specific level.
   *
   * @param level - Storage level
  ...
  private async listSkillsAtLevel(level: SkillLevel): Promise<SkillConfig[]> {
    ...
    if (level === 'bundled') {
      ...
      const bundledSkills = await this.loadSkillsFromDir(bundledDir, 'bundled');
      debugLogger.debug(`Loaded ${bundledSkills.length} bundled skills`);
