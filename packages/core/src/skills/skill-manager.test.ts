...
describe('SkillManager', () => {
  ...
  const validSkillConfig: SkillConfig = {
    ...
    description: 'A test skill',
    level: 'project',
    filePath: '/test/project/.rdmind/skills/test-skill/SKILL.md',
...
`;
    ...
    it('should determine level from file path', () => {
      const projectPath = '/test/project/.rdmind/skills/test-skill/SKILL.md';
      const userPath = '/home/user/.rdmind/skills/test-skill/SKILL.md';
...
Skill 3 content`);
  ...
  describe('getSkillsBaseDir', () => {
    it('should return project-level base dir', () => {
      ...
      expect(baseDir).toBe(path.join('/test/project', '.rdmind', 'skills'));
    });

    it('should return user-level base dir', () => {
      const baseDir = manager.getSkillsBaseDir('user');

      expect(baseDir).toBe(path.join('/home/user', '.rdmind', 'skills'));
    });
  });
