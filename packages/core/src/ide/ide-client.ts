...
export class IdeClient {
  ...
  async connect(): Promise<void> {
    ...
    const workspacePath =
      ...
      process.env['RDMIND_CODE_IDE_WORKSPACE_PATH'];
  ...
  static validateWorkspacePath(
    ...
    if (!isWithinWorkspace) {
      return {
        isValid: false,
        error: `Directory mismatch. RDMind is running in a different location than the open workspace in the IDE. Please run the CLI from one of the following directories: ${ideWorkspacePaths.join(
          ', ',
        )}`,
      };
    }
  ...
  private getStdioConfigFromEnv(): StdioConfig | undefined {
    const command = process.env['RDMIND_CODE_IDE_SERVER_STDIO_COMMAND'];
    if (!command) {
      return undefined;
    }
    ...
    if (argsStr) {
      try {
        ...
        logger.error('Failed to parse RDMIND_CODE_IDE_SERVER_STDIO_ARGS:', e);
      }
    }
  ...
  > {
    ...
    try {
      ...
      // /^rdmind-code-ide-server-${pid}-\d+\.json$/. If multiple IDE
