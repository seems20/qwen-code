...
describe('debugLogger', () => {
  ...
  const previousDebugLogFileEnv = process.env['RDMIND_DEBUG_LOG_FILE'];

  beforeEach(() => {
    process.env['RDMIND_DEBUG_LOG_FILE'] = '1';
    vi.clearAllMocks();
    vi.useFakeTimers();
  ...
  afterEach(() => {
    ...
    if (previousDebugLogFileEnv === undefined) {
      delete process.env['RDMIND_DEBUG_LOG_FILE'];
    } else {
      process.env['RDMIND_DEBUG_LOG_FILE'] = previousDebugLogFileEnv;
    }
  });
  ...
  describe('latest debug log symlink', () => {
    ...
    it('does not create symlink when debug logging is disabled', async () => {
      process.env['RDMIND_DEBUG_LOG_FILE'] = '0';
      vi.clearAllMocks();
      resetDebugLoggingState();
      setDebugLogSession(mockSession);
