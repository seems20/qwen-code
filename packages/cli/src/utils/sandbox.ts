...
import type { Config, SandboxConfig } from '@rdmind/rdmind-core';
import { FatalSandboxError } from '@rdmind/rdmind-core';
    ...
    if (!gcPath.includes('qwen-code/packages/')) {
      throw new FatalSandboxError(
        'Cannot build sandbox using installed RDMind binary; ' +
          'run `npm link ./packages/cli` under RDMind-cli repo to switch to linked binary.',
      );
    } else {
