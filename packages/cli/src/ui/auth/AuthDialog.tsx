...
type MainOption = typeof AuthType.XHS_SSO | typeof AuthType.QWEN_OAUTH | 'CODING_PLAN' | 'API_KEY';
...
export function AuthDialog(): React.JSX.Element {
  ...
  const mainItems = [
    {
      ...
      title: t('小红书 SSO'),
      label: t('小红书 SSO'),
      description: t('Login with Xiaohongshu SSO'),
      value: AuthType.XHS_SSO as MainOption,
    },
    // 暂时隐藏 Qwen OAuth
    // {
    ...
    // },
    // 暂时隐藏 Alibaba Cloud Coding Plan
    // {
    //   key: 'CODING_PLAN',
  ...
  const authTypeToMainOption = (authType: AuthType): MainOption => {
    if (authType === AuthType.XHS_SSO) return AuthType.XHS_SSO;
  ...
  const handleMainSelect = async (value: MainOption) => {
    ...
    // For XHS SSO / Qwen OAuth, proceed directly
    await onAuthSelect(value as AuthType);
  };
  ...
  useKeypress(
    (key) => {
      // Handle Enter key for custom-info view to trigger OpenAI config
      if (key.name === 'return' && viewLevel === 'custom-info') {
        onAuthSelect(AuthType.USE_OPENAI);
        return;
      }
