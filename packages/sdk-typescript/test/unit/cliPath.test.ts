...
describe('CLI Path Utilities', () => {
  ...
  describe('prepareSpawnInfo', () => {
    ...
    describe('command name detection', () => {
      ...
      it('should detect command names on Windows', () => {
        const result = prepareSpawnInfo('rdmind.exe');

        expect(result).toEqual({
          command: 'rdmind.exe',
          args: [],
          type: 'native',
          originalInput: 'rdmind.exe',
        });
      });

      it('should reject invalid command name characters', () => {
        expect(() => prepareSpawnInfo('rdmind@invalid')).toThrow(
      ...
      it('should accept valid command names', () => {
        expect(() => prepareSpawnInfo('rdmind')).not.toThrow();
        expect(() => prepareSpawnInfo('rdmind-code')).not.toThrow();
        expect(() => prepareSpawnInfo('rdmind_code')).not.toThrow();
        expect(() => prepareSpawnInfo('rdmind.exe')).not.toThrow();
        expect(() => prepareSpawnInfo('rdmind123')).not.toThrow();
      });
    });
    ...
    describe('file path resolution', () => {
      it('should resolve absolute file paths', () => {
        ...
        const result = prepareSpawnInfo('/absolute/path/to/rdmind');

        expect(result.command).toBe(path.resolve('/absolute/path/to/rdmind'));
  ...
  describe('Windows path handling', () => {
    ...
    it('should handle Windows native executables', () => {
      const windowsPath = 'C:\\Program Files\\rdmind\\rdmind.exe';
      const result = prepareSpawnInfo(windowsPath);
