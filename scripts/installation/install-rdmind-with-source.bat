@echo off
REM Script to install Node.js and RDMind with source information
REM This script handles the installation process and sets the installation source
REM
REM Usage: install-rdmind-with-source.bat --source [github|npm|internal|local-build]
REM        install-rdmind-with-source.bat -s [github|npm|internal|local-build]
...
if !ERRORLEVEL! EQU 0 (
    ...
    if !MAJOR_VERSION! GEQ 20 (
        ...
        if !ERRORLEVEL! NEQ 0 (
            echo ERROR: Failed to install Node.js. Cannot continue with RDMind installation.
            exit /b 1
        )
    )
) else (
...
REM Verify npm is available before installing RDMind
...
REM Install RDMind with source information
echo INFO: Installing RDMind with source: %SOURCE%
echo INFO: Running: %NPM_CMD% install -g @rdmind/rdmind
call "%NPM_CMD%" install -g @rdmind/rdmind

if %ERRORLEVEL% EQU 0 (
    echo SUCCESS: RDMind installed successfully!
) else (
    echo ERROR: Failed to install RDMind.
    exit /b 1
)

REM After installation, create source.json in the .rdmind directory
echo INFO: Creating source.json in %USERPROFILE%\.rdmind...

set "RDMIND_DIR=%USERPROFILE%\.rdmind"
if not exist "%RDMIND_DIR%" (
    mkdir "%RDMIND_DIR%"
)

REM Create the source.json file with the installation source
echo { > "%RDMIND_DIR%\source.json"
echo   "source": "%SOURCE%" >> "%RDMIND_DIR%\source.json"
echo } >> "%RDMIND_DIR%\source.json"

echo SUCCESS: Installation source saved to %USERPROFILE%\.rdmind\source.json

REM Verify installation
call :CheckCommandExists rdmind
if %ERRORLEVEL% EQU 0 (
    echo SUCCESS: RDMind is available as 'rdmind' command.
    call rdmind --version
) else (
    echo WARNING: RDMind may not be in PATH. Please check your npm global bin directory.
)

echo.
echo ===========================================
echo SUCCESS: Installation completed!
echo The source information is stored in %USERPROFILE%\.rdmind\source.json
echo.
echo ===========================================
...
set "TEMP_DIR=%TEMP%\rdmind-nodejs-install"
